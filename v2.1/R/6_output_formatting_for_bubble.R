...
nyc_buildings_csv = nyc_buildings_nona %>% dplyr::select(h_no,hno_suffix,full_stree,zipcode,`TOPSIS MCDA Rank (1 = Best)`,`TOPSIS score`,`Flat, Usable Area (ft2)`,FAID) %>%
...
nyc_buildings_csv = nyc_buildings_csv %>% dplyr::select(c(address_short,address_geo,address_text,application,bids,company,cre_report,
                                                          energy_reduction_electricity,
                                                          energy_reduction_heating,energy_savings,energy_savings_score_verbal,FAID,financing,
                                                          ...
                                                          stormwater_capture_score_verbal,stormwater_mngt_savings,total_cost_savings,
...
nyc_buildings_csv_inclna = nyc_buildings %>% dplyr::select(h_no,hno_suffix,full_stree,zipcode,`TOPSIS MCDA Rank (1 = Best)`,`TOPSIS score`,`Flat, Usable Area (ft2)`,FAID) %>%
...
nyc_buildings_csv_inclna = nyc_buildings_csv_inclna %>% dplyr::select(c(address_short,address_geo,address_text,application,bids,company,cre_report,
                                                                        energy_reduction_electricity,
                                                           energy_reduction_heating,energy_savings,energy_savings_score_verbal,FAID,financing,
                                                           greenroof_installation_cost,greenroof_maintenance_costs,img_map,
                                                           members,name,notified,roi,score,score_verbal,start_date,status,stormwater_capture,
...
nyc_buildings_csv_inclna=nyc_buildings_csv_inclna[-c(which(nyc_buildings_csv_inclna$address_short=="516 E 13 St"),which(nyc_buildings_csv_inclna$address_short=="922 Prospect Pl")),]
